...
class Fecha
{
	...
	public:
                ...
                string imprimir();
                string obtenerdiasem(int d, int m, int a);
                int zeller(int y, int m, int d);

	        //Gets
		int getdia(){return f->dia;}
		int getmes(){return f->mes;}
		int getano(){return f->ano;}
		int gethora(){return f->hora;}
		int getminuto(){return f->minuto;}
};
...
void Fecha::ponerfechaact()
{
	...
	hora=0;
	minuto=0;
	diasem=obtenerdiasem(f->dia,f->mes,f->ano);
}
...
bool Fecha::igualque(Fecha *comp)
{
	...
	if ((ano == comp->getano()) && (fmes == comp->getmes())
            && (dia == comp->getdia()) && (hora == comp->gethora())
            && (fminuto == comp->getminuto()))
...
void Fecha::agregar(int d, int me, int a, int h, int mi)
{
	...
	temp=a+ano;
	ano=temp;
        diasem=obtenerdiasem(dia,mes,ano);
}
...
/*
* Retorna el valor en string la fecha y hora
* */
string Fecha::imprimir()
{
	string res("[ "+diasem+" "+to_string(dia)+"/"+to_string(mes)+
...
* */
string Fecha::obtenerdiasem(int d, int m, int a)
{
        int res;
        string dia_sem;
        if (m < 3) a = a - 1;
